...
bin/dose-project.sh          } implementations `dose` execs. They are NOT verbs
bin/freeze-check.sh          } and are not on PATH -- each has a row in
bin/verdict.sh               } realisateur's bin/lib/not-a-verb.tsv. They travel
bin/usage-paced-runner.sh    } here so the verb BUILD carries them beside
lib/dose-common.sh           } bin/dose, which is what lets $SELF find them on a
                             } host with no scheduler checkout at all.
```

Those five arrived with hf7y/scheduler#123 and #127. Before them this branch
really did hold "a plain shell utility and nothing else", and `dose` was a
facade: every subcommand exec'd into a checkout via LEGACY_ROOT, so on a host
with no clone the whole verb GAPped. Zach, 2026-08-11: *"scheduler should not
need to exist as a check out on monkey for the verbs to work ... that's true for
all verbs actually, they should be functional independent of their underlying
repos."*
