...
*house and version the home automation's configuration*

This is the **bashified** branch of `home-assistant`. It contains a plain shell
utility and nothing else.

```
bin/loge          the utility
man/loge.1        how to use it
...
the tool itself is gone from these files. It is not lost: it is on `master`
branch of this same repository, one `git log master` away.
...
```
./test/contract-test.sh bin/loge
```
